...
})({"dVRbT":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
function downloadThumbnail(e) {
    _htmlToImage.toJpeg(e.currentTarget, {
        quality: 0.95
...
document.querySelector('.wrapper').addEventListener('click', downloadThumbnail);
...
function getImageSize(node, options = {}) {
    const width = options.width || _util.getNodeWidth(node);
    const height = options.height || _util.getNodeHeight(node);
    return {
        width,
        height
    };
...
function toSvg(node, options = {}) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        return Promise.resolve(node).then((nativeNode)=>_cloneNode.cloneNode(nativeNode, options, true)
        ).then((clonedNode)=>_embedWebFonts.embedWebFonts(clonedNode, options)
        ).then((clonedNode)=>_embedImages.embedImages(clonedNode, options)
        ).then((clonedNode)=>_applyStyleWithOptions.applyStyleWithOptions(clonedNode, options)
        ).then((clonedNode)=>_util.nodeToDataURL(clonedNode, width, height)
        );
    });
}
...
function toCanvas(node, options = {}) {
    return __awaiter(this, void 0, void 0, function*() {
        return toSvg(node, options).then(_util.createImage).then((img)=>{
            const canvas = document.createElement('canvas');
            const context = canvas.getContext('2d');
            const ratio = options.pixelRatio || _util.getPixelRatio();
            const { width , height  } = getImageSize(node, options);
...
function toBlob(node, options = {}) {
    return __awaiter(this, void 0, void 0, function*() {
        return toCanvas(node, options).then(_util.canvasToBlob);
    });
}
function getFontEmbedCSS(node, options = {}) {
    return __awaiter(this, void 0, void 0, function*() {
        return _embedWebFonts.getWebFontCSS(node, options);
    });
}
...
function cloneCanvasElement(node) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        return _util.createImage(dataURL);
    });
}
function cloneVideoElement(node, options) {
    return __awaiter(this, void 0, void 0, function*() {
        return Promise.resolve(node.poster).then((url)=>_getBlobFromURL.getBlobFromURL(url, options)
        ).then((data)=>_util.makeDataUrl(data.blob, _util.getMimeType(node.poster) || data.contentType)
        ).then((dataURL)=>_util.createImage(dataURL)
        );
    });
}
...
function cloneChildren(nativeNode, clonedNode, options) {
    var _a;
    return __awaiter(this, void 0, void 0, function*() {
        const children = isSlotElement(nativeNode) && nativeNode.assignedNodes ? _util.toArray(nativeNode.assignedNodes()) : _util.toArray(((_a = nativeNode.shadowRoot) !== null && _a !== void 0 ? _a : nativeNode).childNodes);
...
function cloneCSSStyle(nativeNode, clonedNode) {
    ...
    else _util.toArray(source).forEach((name)=>{
...
function decorate(nativeNode, clonedNode) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        return Promise.resolve().then(()=>cloneCSSStyle(nativeNode, clonedNode)
        ).then(()=>_clonePseudoElements.clonePseudoElements(nativeNode, clonedNode)
...
function cloneNode(node, options, isRoot) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        return Promise.resolve(node).then((clonedNode)=>cloneSingleNode(clonedNode, options)
...
function getBlobFromURL(url, options) {
    ...
    const deferred = window.fetch(url).then((res)=>// eslint-disable-next-line promise/no-nesting
        res.blob().then((blob)=>({
            ...
            contentType,
            blob: _util.parseDataUrlContent(blob)
        })
    )// on failed
    .catch(failed);
    // cache result
...
const delay = (ms)=>(args)=>new Promise((resolve)=>setTimeout(()=>resolve(args)
            , ms)
        )
;
...
function svgToDataURL(svg) {
    return __awaiter(this, void 0, void 0, function*() {
        return Promise.resolve().then(()=>new XMLSerializer().serializeToString(svg)
        ).then(encodeURIComponent).then((html)=>`data:image/svg+xml;charset=utf-8,${html}`
        );
    });
}
...
function formatCSSProperties(style) {
    return _util.toArray(style).map((name)=>{
...
function embedBackground(clonedNode, options) {
    ...
    return __awaiter(this, void 0, void 0, function*() {
        ...
        return Promise.resolve(background).then((cssString)=>_embedResources.embedResources(cssString, null, options)
        ).then((cssString)=>{
...
function embedImageNode(clonedNode, options) {
    return __awaiter(this, void 0, void 0, function*() {
        if (!(clonedNode instanceof HTMLImageElement && !_util.isDataUrl(clonedNode.src)) && !(clonedNode instanceof SVGImageElement && !_util.isDataUrl(clonedNode.href.baseVal))) return Promise.resolve(clonedNode);
        const src = clonedNode instanceof HTMLImageElement ? clonedNode.src : clonedNode.href.baseVal;
        return Promise.resolve(src).then((url)=>_getBlobFromURL.getBlobFromURL(url, options)
        ).then((data)=>_util.makeDataUrl(data.blob, _util.getMimeType(src) || data.contentType)
...
function embedChildren(clonedNode, options) {
    return __awaiter(this, void 0, void 0, function*() {
        const children = _util.toArray(clonedNode.childNodes);
...
function embedImages(clonedNode, options) {
    return __awaiter(this, void 0, void 0, function*() {
        if (!(clonedNode instanceof Element)) return Promise.resolve(clonedNode);
        return Promise.resolve(clonedNode).then((node)=>embedBackground(node, options)
...
function parseURLs(cssText) {
    ...
    return result.filter((url)=>!_util.isDataUrl(url)
    );
}
function embed(cssText, resourceURL, baseURL, options, get) {
    const resolvedURL = baseURL ? _util.resolveUrl(resourceURL, baseURL) : resourceURL;
    return Promise.resolve(resolvedURL).then((url)=>get ? get(url) : _getBlobFromURL.getBlobFromURL(url, options)
    ).then((data)=>{
        if (typeof data === 'string') return _util.makeDataUrl(data, _util.getMimeType(resourceURL));
        return _util.makeDataUrl(data.blob, _util.getMimeType(resourceURL) || data.contentType);
    ...
    return __awaiter(this, void 0, void 0, function*() {
        ...
        styleSheets.forEach((sheet)=>{
            if ('cssRules' in sheet) try {
                _util.toArray(sheet.cssRules).forEach((item, index)=>{
                    if (item.type === CSSRule.IMPORT_RULE) {
                        let importIndex = index + 1;
                        const url = item.href;
                        const deferred = fetchCSS(url).then((metadata)=>metadata ? embedFonts(metadata) : ''
                ...
                const inline = styleSheets.find((a)=>a.href == null
                ) || document.styleSheets[0];
                if (sheet.href != null) deferreds.push(fetchCSS(sheet.href).then((metadata)=>metadata ? embedFonts(metadata) : ''
                ).then((cssText)=>parseCSS(cssText).forEach((rule)=>{
        ...
        return Promise.all(deferreds).then(()=>{
            ...
            styleSheets.forEach((sheet)=>{
                if ('cssRules' in sheet) try {
                    _util.toArray(sheet.cssRules).forEach((item)=>{
                        ret.push(item);
                    });
                } catch (e) {
...
function getWebFontRules(cssRules) {
    return cssRules.filter((rule)=>rule.type === CSSRule.FONT_FACE_RULE
    ).filter((rule)=>_embedResources.shouldEmbed(rule.style.getPropertyValue('src'))
    );
}
function parseWebFontRules(node) {
    return __awaiter(this, void 0, void 0, function*() {
        return new Promise((resolve, reject)=>{
            ...
            resolve(_util.toArray(node.ownerDocument.styleSheets));
...
function getWebFontCSS(node, options) {
    return __awaiter(this, void 0, void 0, function*() {
        return parseWebFontRules(node).then((rules)=>Promise.all(rules.map((rule)=>{
                ...
                return _embedResources.embedResources(rule.cssText, baseUrl, options);
            }))
...
},{"./util":"eIddd","./embedResources":"iQ21R","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["dVRbT","iI2BD"], "iI2BD", "parcelRequire1482")
